...
static void stripmarkup(char *dst, const char *src, size_t size) {
    size_t o = 0;
...
static void addline(Notif *n, const char *s, int len) {
    if (n->nlines >= (int)maxlines || n->len + len + 2 > (int)sizeof n->text)
        return;
...
static int textw(const char *s, int len) {
    XGlyphInfo ext;
...
static void wrapappend(Notif *n, const char *s, int avail) {
    int w, cw, cl, len, brk;
...
static void settext(Notif *n, const char *summary, const char *body) {
...
static void makewin(Notif *n) {
    ...
    n->win = XCreateWindow(
        dpy, root, 0, 0, (unsigned int)n->w, (unsigned int)n->h, borderpx,
...
static void layout(void) {
    ...
    for (i = 0; i < n; i++) {
        ...
        XMoveResizeWindow(dpy, notifs[i].win, x, y, (unsigned int)notifs[i].w,
...
static void draw(Notif *n) {
    ...
    while ((e = strchr(s, '\n'))) {
        XftDrawStringUtf8(
            xd, &xftpixels[n->urgency][i < n->sumlines ? ColSum : ColBody],
            font, (int)hpad, y, (FcChar8 *)s, (int)(e - s));
        s = e + 1;
        y += lineh;
        i++;
    }
...
static void signalclosed(unsigned int id, unsigned int reason) {
    DBusMessage *sig;
    dbus_uint32_t i = id, r = reason;

    if (!(sig = dbus_message_new_signal(NPATH, NIFACE, "NotificationClosed")))
        return;
    dbus_message_append_args(sig, DBUS_TYPE_UINT32, &i, DBUS_TYPE_UINT32, &r,
                             DBUS_TYPE_INVALID);
...
static void signalaction(unsigned int id) {
    DBusMessage *sig;
    dbus_uint32_t i = id;
    ...
    if (!(sig = dbus_message_new_signal(NPATH, NIFACE, "ActionInvoked")))
        return;
    dbus_message_append_args(sig, DBUS_TYPE_UINT32, &i, DBUS_TYPE_STRING, &key,
                             DBUS_TYPE_INVALID);
    dbus_connection_send(conn, sig, NULL);
    dbus_connection_flush(conn);
    dbus_message_unref(sig);
...
static void closenotif(int i, unsigned int reason) {
...
static void closeall(unsigned int reason) {
    int i;
...
static void sendmsg(DBusMessage *msg) {
    dbus_connection_send(conn, msg, NULL);
    dbus_connection_flush(conn);
    dbus_message_unref(msg);
}

static void senderr(DBusMessage *msg, const char *name, const char *text) {
    DBusMessage *reply;
...
static void notify(DBusMessage *msg) {
...
static void closecall(DBusMessage *msg) {
    DBusMessage *reply;
    dbus_uint32_t id;
    int i;
...
static void capabilities(DBusMessage *msg) {
    DBusMessage *reply;
...
static void serverinfo(DBusMessage *msg) {
    DBusMessage *reply;
...
static void introspect(DBusMessage *msg) {
    DBusMessage *reply;
...
static void handlemsg(DBusMessage *msg) {
    if (dbus_message_is_method_call(msg, NIFACE, "Notify"))
        notify(msg);
    else if (dbus_message_is_method_call(msg, NIFACE, "CloseNotification"))
        closecall(msg);
    else if (dbus_message_is_method_call(msg, NIFACE, "GetCapabilities"))
        capabilities(msg);
    else if (dbus_message_is_method_call(msg, NIFACE, "GetServerInformation"))
        serverinfo(msg);
...
static void handle(XEvent *ev) {
    ...
    switch (ev->type) {
    case Expose:
        if (ev->xexpose.count == 0 && (i = findwin(ev->xexpose.window)) >= 0)
            draw(&notifs[i]);
        break;
    case ButtonPress:
        ...
        if (ev->xbutton.button == Button3) {
            ...
            if (ev->xbutton.button == Button1 && notifs[i].hasdefault)
    ...
    case ConfigureNotify:
        if (ev->xconfigure.event == root && findwin(ev->xconfigure.window) < 0)
            raiseall();
        break;
    case MapNotify:
        if (ev->xmap.event == root && findwin(ev->xmap.window) < 0)
            raiseall();
        break;
    }
}
...
static void setup(void) {
    ...
    if (dbus_bus_request_name(conn, NIFACE, DBUS_NAME_FLAG_DO_NOT_QUEUE,
                              &err) != DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER)
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j < 4; j++) {
            ...
            if (!XftColorAllocName(dpy, DefaultVisual(dpy, screen),
                                   DefaultColormap(dpy, screen), colors[i][j],
                                   &xftpixels[i][j]))
...
static void run(void) {
    ...
    for (;;) {
        ...
        next = 0;
        for (i = 0; i < (int)arrlen(notifs); i++)
            if (notifs[i].deadline && (!next || notifs[i].deadline < next))
        ...
        if (select(nfds, &fds, NULL, NULL, ptv) < 0 && errno != EINTR)
